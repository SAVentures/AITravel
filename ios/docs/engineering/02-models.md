...
> ### Naming + placement — name and directory tell you the kind
> | Kind | Suffix | Lives in | Example |
> |---|---|---|---|
> | **Leaf value type** | none (bare) | `Models/` | `City`, `Author`, `Rating` |
> | **Reference model** (`@Observable`) | **`Model`** | `Models/` | `BookModel`, `TripDraftModel` |
> | **Wire DTO** | **`DTO`** | `Networking/Responses/DTO/` | `BookDTO`, `LibraryDTO` |
> | **Request** | **`<Verb><Name>Request`** | `Networking/Requests/` | `GetLibraryRequest` |
>
> The entity name is bare; only the reference/wire twins carry a suffix (`BookModel` ↔ `BookDTO`), never a
> prefix. Seed lives in `Models/SampleData/`. `AppStore+<Domain>` / `SampleData+<Domain>` files are named
> for the domain, not a model (`AppStore+Library.swift`, not `+LibraryModel`).
...
> **Every wire value type is `nonisolated`.** Under MainActor-by-default a bare `struct Author: Codable`
> is MainActor-isolated and can't decode off-main — and the error surfaces at the composing DTO, not the
> leaf. So every DTO and every leaf value type a DTO composes is `nonisolated`. Full rule: `04-networking.md §2`.
