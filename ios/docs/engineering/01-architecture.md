...
| **The boundary is `nonisolated`, not just `Sendable`** | DTOs, leaf value types, the `APIRequest`/`APIClientProtocol` requirements, request structs, `LiveProvider`, and `APIJSON` decode off-main, so each is `nonisolated` (`Sendable` alone leaves it MainActor-isolated → won't compile). Reference models aren't `Codable`; `AppStore` maps `dto.toDomain()` on main. Rule: `04-networking.md §2`. |
