...
| **The boundary is what you mark** | DTOs + leaf value types + `APIRequest`/`APIClientProtocol` are `Sendable` value/protocol types that cross to the background. Reference models are **not** `Codable` and never decode off-actor; `AppStore` maps `dto.toDomain()` back on the main actor (the default). |
